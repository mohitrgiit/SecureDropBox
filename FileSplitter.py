...
class FileSplitter:
	...
	extrabytes = fsize%3
        output_filename = self.__filename
        commandstring = "dd if=/dev/zero bs=1 count=%s >> %s " % (extrabytes,output_filename)
        #commandstring ="echo %s >> log.txt" % (self.__filename)
        print "padding file"
        ...
        for i in range(6):
            randomfilename =  self.__filename + "R" + str(i+1) 
            commandstring = "dd if=/dev/urandom of=%s bs=1 count=%s " % (randomfilename,randomfilesize)
            os.system(commandstring)
