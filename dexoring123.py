...
# xoring to get r4,5,6,7,8,9 if server 1,2,3 is available and server 4 is down.
def xor_s123_1(input1, input2):
	...
	str2 = file2.read()
        ...
        # r1 ^ r14 = r4
	if(input1.endswith("R1") and input2.endswith("R14")):
                filename = input1.replace("R1","R4") 
         	print filename
		r4 = sxor(str1,str2)
	...
	# r2 ^ r26 = r6
		...
		print "Xoring done " + filename
	# r3 ^ r38 = r8
	elif(input1.endswith("R3") and input2.endswith("R38")):
                filename = input1.replace("R3","R8")
         	print filename
		r8 = sxor(str1,str2)
		file3 = open(filename, "w")
		file3.write(r8)
		print "Xoring done " + filename
	# r6 ^ r67 = r7
	if(input1.endswith("R6") and input2.endswith("R67")):
                filename = input1.replace("R6","R7")
         	print filename
		r7 = sxor(str1,str2)
		file3 = open(filename, "w")
		file3.write(r7)
		print "Xoring done " + filename
	# r8 ^ r89 = r9
        elif(input1.endswith("R8") and input2.endswith("R89")):
                filename = input1.replace("R8","R9")
         	print filename
		r9 = sxor(str1,str2)
		file3 = open(filename, "w")
		file3.write(r9)
		print "Xoring done " + filename
	# r4 ^ r45 = r5
	elif(input1.endswith("R4") and input2.endswith("R45")):
                filename = input1.replace("R4","R5")
         	print filename
		r5 = sxor(str1,str2)
		file3 = open(filename, "w")
		file3.write(r5)
        ...
        for p in L_Ri: 
          for q in L_Rij:
	       ...
	       if(p.endswith("R2") and (q.endswith("R26"))):
                       xor_s123_1(p,q)
	       if(p.endswith("R3") and (q.endswith("R38"))):
                       xor_s123_1(p,q)
	...
	for files in os.listdir("."):
        ...
        for p in L3: 
          for q in L_Rij:
               ...
               if(p.endswith("R6") and (q.endswith("R67"))):
                       xor_s123_1(p,q)
	       if(p.endswith("R8") and (q.endswith("R89"))):
                       xor_s123_1(p,q)
	       if(p.endswith("R4") and (q.endswith("R45"))):
                       xor_s123_1(p,q)  
